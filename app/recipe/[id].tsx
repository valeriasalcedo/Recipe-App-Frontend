...
const RecipeDetailScreen = () => {
  const { id: recipeIdParam } = useLocalSearchParams();
  ...
  useEffect(() => {
    ...
    const checkSaved = async () => {
      if (!userId) return setIsSaved(false);
      const saved = await isFavorite(userId, recipeId);
      setIsSaved(saved);
    };
    ...
    const loadRecipeDetail = async () => {
      setLoading(true);
      try {
        const mealData = await MealAPI.getMealById(String(recipeId));
        if (mealData) {
  ...
  const handleToggleSave = async () => {
    if (!userId) {
      Alert.alert("Login required", "Please sign in to use favorites.");
      return;
    }
    ...
    try {
      if (isSaved) {
        ...
        await addFavorite(userId, {
          recipeId: Number.isNaN(Number(recipeId)) ? String(recipeId) : Number(recipeId),
          title: recipe.title,
          image: recipe.image,
          ...
          <View style={recipeDetailStyles.floatingButtons}>
            ...
            <TouchableOpacity
              style={[
                ...
                { backgroundColor: isSaving ? COLORS.background : COLORS.primary },
              ]}
